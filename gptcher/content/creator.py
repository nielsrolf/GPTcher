...
def create_exercise(language, exercise_description):
    ...
    if supabase.table("exercises").select("*").eq("language", language).eq("topic", exercise_description["topic"]).execute().data:
        print("Exercise already exists, skipping:", exercise_description["topic"])
        return
    exercise_json = json.dumps(exercise_description, indent=4)
    prompt = exercise_create_prompt.prompt.replace("{exercise_json}", exercise_json).replace("<language>", language)
    exercises_json = complete_and_parse_json(prompt, "\n\n", exercise_create_prompt.prefix, max_tokens=3000)
...
class TranslationTask():
    ...
    def create(language, sentence):
        task = TranslationTask(language, sentence, translate(sentence, language), read_and_save_voice(sentence, language))
        task.to_db()
        return task
    ...
    def from_db(task_id):
        ...
        db_entry = supabase.table("translation_tasks").select("*").eq("id", task_id).execute().data[0]
...
class Exercise():
    ...
    def __post_init__(self):
        if self.id:
            task_ids = supabase.table("exercise_translation_tasks").select("translation_task_id").order("order").eq("exercise_id", self.id).execute().data
            self.translation_tasks = [TranslationTask.from_db(task['translation_task_id']) for task in task_ids]
    
    @staticmethod
    def create(language, topic, content_description, grammar, exercise_number, task_description, vocabulary_en, sentences_en):
        translation_tasks = [TranslationTask.create(language, sentence) for sentence in sentences_en]
        exercise = Exercise(language, topic, content_description, grammar, exercise_number, task_description)
    ...
    def from_db(exercise_id):
        db_entry = supabase.table("exercises").select("*").eq("id", exercise_id).execute().data[0]
...
exercises = supabase.table("exercises").select("*").eq("language", "Spanish").execute().data
