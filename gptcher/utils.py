...
def complete_with_hash(prompt, stop, max_tokens=256, override=False):
    ...
    if not override:
        completions = supabase.table("completions").select("*").eq("prompt_hash", prompt_hash).execute().data
    ...
    if max_tokens > 1000:
        max_tokens = 1000
    supabase.table("completions").upsert({"prompt_hash": prompt_hash, "response": response}).execute()
    return response
